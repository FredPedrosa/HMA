...
> **Repositório Oficial (Research Compendium)** contendo os dados, scripts de análise e materiais suplementares do capítulo sobre o **Modelo Hierárquico do Afeto Musical (HMA)** e a escala **ALMA (AvaLiação Musical do Afeto)**.

## 📌 Sobre o Projeto

Este projeto investiga a arquitetura estrutural da percepção emocional na música, apresentando evidências que sobrepõem o modelo circumplexo bidimensional clássico (Russell, 1980). Por meio de uma extensa triangulação metodológica que contrasta a decodificação algorítmica de Inteligências Artificiais, com os *Audio Transformers* MERT e CLAP, com a cognição humana, o estudo demonstra que o afeto musical opera sob uma arquitetura hierárquica.

A pesquisa demonstra que a **Intensidade** é um **Fator Geral (FG)** substantivo que atua como fundação para a experiência qualitativa. A máquina falha em compreender o afeto ao tentar medir energia de forma estritamente mecânica, gerando a **"Lacuna da Melancolia"**, enquanto o ser humano utiliza um **"Filtro de Serenidade"** — suprimindo ativamente o ruído para construir o relaxamento — e modula sua percepção por meio de diferenças individuais relacionadas à Personalidade e à escoalridade.

### 🌟 Destaques do Repositório

O projeto está dividido em três grandes eixos empíricos:
1. **Estudo 1 (Descoberta Controlada):** Duelo metodológico demonstrando que o framework HMA dobra a capacidade de explicar a acurácia humana em comparação à ipsatização bidimensional clássica. Apresenta o "Filtro da Serenidade".
2. **Estudo 2 (Falsificação em Big Data - DEAM):** Teste em larga escala (1.802 faixas) revelando o colapso preditivo (ΔAIC robustos) do MERT e CLAP quando forçados a operar no espaço 2D sem a magnitude.
3. **Estudo 3 (Calibração Humana e ALMA):** Extração de traços latentes via PLS-SEM de segunda ordem e CFA Bifactor (Fator Geral de Personalidade e *p-factor*). Modelagem reflexivo-formativa (PLS-SEM) validando a escala **ALMA** e provando que a educação suprime e a personalidade amplifica o impacto da música.
...
│   ├── Figura_5.png       # O Filtro da Serenidade (Interação Magnitude x Categoria)
...
* **`seminr`**: Modelagem de Equações Estruturais por Mínimos Quadrados Parciais (PLS-SEM)
...
Pedrosa, F. G. (2026). A Construção Hierárquica do Afeto na Música: Valência e Ativação como Qualidade Residual frente à Intensidade - Repositório. Em preparação / Submetido.
